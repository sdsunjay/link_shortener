class ShortenUrl < ApplicationRecord
  ...
  def generate_short_url
     # url = Array.new(UNIQUE_ID_LENGTH) { LETTERS.sample }.join
     url = LETTERS.shuffle[0,UNIQUE_ID_LENGTH].join

     # check if this is a duplicate
     if ShortenUrl.where(short_url: url).exists?
       # try again
